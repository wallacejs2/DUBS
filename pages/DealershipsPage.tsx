...
import { formatOemsForExport, OEM_HIERARCHY, OEM_MAKES, encodeOemFilter } from '../lib/oem';

// Makes listed alphabetically in the OEM filter dropdown
const OEM_MAKES_SORTED = [...OEM_MAKES].sort((a, b) => a.localeCompare(b));
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative min-w-[130px]">
              <select
                ...
                aria-label="Filter by OEM Group or Make"
                className="w-full pl-2.5 pr-7 py-1.5 bg-slate-100/50 dark:bg-[#1C1C1E] border border-slate-200/60 dark:border-[#38383A] rounded-xl text-sm text-slate-700 dark:text-slate-300 appearance-none focus:ring-1 focus:ring-blue-500 outline-none cursor-pointer"
              >
                <option value="">All OEMs</option>
                <optgroup label="OEM Groups">
                  {OEM_HIERARCHY.map(g => <option key={g.group} value={encodeOemFilter('group', g.group)}>{g.group}</option>)}
                </optgroup>
                <optgroup label="Makes">
                  {OEM_MAKES_SORTED.map(m => <option key={m} value={encodeOemFilter('make', m)}>{m}</option>)}
                </optgroup>
              </select>
              <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" size={12} />
            </div>
        ...
        <div className="flex-1 w-full min-w-0">
            {loading ? (
                ...
                {dealerships.map(dealer => {
                    ...
                    const missingOem = !dealer.oems || dealer.oems.length === 0;
                    return (
                    <DealershipCard
                        key={dealer.id}
                        dealership={dealer}
