        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white/95 dark:bg-[#1C1C1E]/95 custom-scrollbar transition-colors">
          <div className="space-y-6">
            ...
            <div className={`grid ${isProvider ? 'grid-cols-2' : 'grid-cols-1'} gap-4 transition-all duration-200`}>
               <div>
                  ...
                  {isEditing ? (
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-sm font-semibold border ${categoryColors[formData.category || ProviderProductCategory.PROVIDER]}`}>
                      {formData.category}
                    </span>
                  )}
               </div>
               ...
               {isProvider && (
                   <div className="animate-in fade-in zoom-in-95 duration-200">
                    ...
                    {isEditing ? (
                        ...
                        <span className="px-2.5 py-1 rounded-full text-sm font-semibold border bg-slate-50 border-slate-200 text-slate-600 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400">
            ...
            <div className="pt-4 border-t border-slate-100/60 dark:border-[#38383A]">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">Support Information</h3>
              <div className="space-y-4">
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <DataValue>
                      <a href={`mailto:${formData.support_email}`} className="text-blue-600 dark:text-blue-400 hover:underline">{formData.support_email}</a>
                    </DataValue>
                  )}
                </div>
                <div>
                  ...
                  {isEditing ? (
                    ...
                    <DataValue>
                      {formData.support_link ? (
                        <a href={formData.support_link} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1">
            ...
            <div className="pt-4 border-t border-slate-100/60 dark:border-[#38383A]">
               ...
               {isEditing ? (
                   <textarea 
                     ...
                     className="w-full px-3 py-2 text-sm border border-slate-200/60 dark:border-[#38383A] rounded-xl focus:ring-1 focus:ring-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500/50 outline-none bg-slate-100/50 dark:bg-[#2C2C2E] text-slate-900 dark:text-slate-100 font-normal min-h-[80px]"
                     placeholder="Add any internal context..."
                   />
               ) : (
                   <div className="text-sm text-slate-600 dark:text-slate-400 italic bg-slate-50/50 dark:bg-white/[0.02] p-3 rounded-lg border border-slate-100/60 dark:border-[#38383A]">
                                ...
                                <div key={d.id} className={`flex items-center justify-between p-2.5 bg-slate-50/50 dark:bg-white/[0.02] rounded-lg border ${d.status === DealershipStatus.CANCELLED ? 'border-red-100 dark:border-red-900/30 bg-red-50/10 dark:bg-red-900/10' : 'border-slate-100 dark:border-slate-700'}`}>
                                    <div className="min-w-0 flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div className="text-xs font-mono text-slate-400 dark:text-slate-500">{d.pp_sys_id || 'NO ID'}</div>
                                        </div>
                                    </div>
